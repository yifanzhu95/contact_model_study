...
horizon lengths (HORIZONS) and the MODELS inline, maps $SLURM_ARRAY_TASK_ID to
one horizon value, and calls this worker once per model. This worker runs
`--n_episodes` episodes for that one cell via `run_eval_episode` (no video),
aggregates them into an AggregatedResult labelled "<model>_h<horizon>", and
writes ONE JSON to `--outdir` in the format the directory sweep plotter reads
(analysis/plot_horizon_sweep_dir.py).

    python run_horizon_cell.py \
        --outdir results/horizon_eval_run \
        --task grasp_reorient --model M2 --horizon 24 \
        --n_episodes 5
"""
...
# ---------------------------------------------------------------------------
def run_cell(args):
    """Run one (model, horizon) cell (n_episodes episodes) -> AggregatedResult."""
    ...
    label    = f"{args.model}_h{args.horizon}"
    ...
    seed_seq  = np.random.SeedSequence(
        [s for s in (args.seed, args.horizon, model_ord) if s is not None] or None
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--horizon",    type=int, required=True,
                   help="MPPI planning horizon for this cell (the swept axis).")
    ...
    # --- MPPI / eval knobs (all command-line inputs; n_samples/substeps fixed) --
    ...
    p.add_argument("--substeps",      type=int,   default=16,
                   help="MPPI rollout substeps per control step (held constant across the sweep).")
