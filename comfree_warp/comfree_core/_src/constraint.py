...
from . import support
from . import types
from .types import SPARSE_CONSTRAINT_JACOBIAN
...
def _efc_row(
  ...
  # comfree only outputs:
  efc_dist_out: wp.array2d(dtype=float),
  efc_mass_out: wp.array2d(dtype=float),
):
  # calculate kbi
  timeconst = solref[0]
  dampratio = solref[1]
...
#   width = wp.max(types.MJ_MINVAL, width)
  width = 0.01 # Set width to a constant value to avoid signficant jump when pos_imp is large (e.g., initial penetration)
  ...
  # comfree only outputs
  efc_dist_out[worldid, efcid] = pos_aref + margin
  efc_mass_out[worldid, efcid] = 1.0 / wp.max(invweight * (1.0 - imp) / imp, types.MJ_MINVAL)
  ...
  if not (m.opt.disableflags & types.DisableBit.CONSTRAINT):
    ...
    if not (m.opt.disableflags & types.DisableBit.CONTACT):
      if m.opt.cone == types.ConeType.PYRAMIDAL:
        ...
        raise NotImplementedError("Only pyramidal contact is currently implemented.")
